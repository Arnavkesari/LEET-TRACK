...
const LandingPage = () => {
  ...
  const handleGoogleAuth = async () => {
    try {
      ...
      const googleAuthUrl = authUrlResponse.authUrl;
      ...
      const handleMessage = async (event) => {
        ...
        if (event.data.type === 'GOOGLE_AUTH_SUCCESS') {
          popup.close();
          try {
            // The event.data should contain the token, not user data
            const result = await googleLogin(event.data.token || event.data.user);
            if (result.success) {
              navigate('/dashboard');
            } else {
      ...
      // Clean up timeout when popup closes or succeeds
      const originalHandleMessage = handleMessage;
      const wrappedHandleMessage = async (event) => {
